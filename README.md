...
当前 Compose 发布 `8445:8445`。宿主机反代应将端口映射改为 `127.0.0.1:8445:8445`，避免公网绕过代理；反代位于其他容器时使用私有 Docker 网络，并通过防火墙限制直连来源。
...
- **大厅密码即访问凭证**。大厅 ID 只由名称决定；服务端单独校验已有大厅的密码，错误时不下发聊天令牌或成员信息。请使用不易猜测的密码。大厅随最后一位成员离开而销毁，并非永久保留的账号或房间。
...
- **公网部署必须用 WSS**。明文 WS 会把信令内容（包括大厅密码）暴露在链路上。
  当前 Compose 的 `8445:8445` 需要按上述反代部署建议收紧到回环映射或受限私有网络。
